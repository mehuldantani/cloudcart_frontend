...
const Createcoupon = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try{
      const resp = await axios.put(`${process.env.REACT_APP_BASE_URL}api/v1/coupon/${selected._id}`,{
        "code":updatedcode,
  ...
  const handleDelete = async (itemid) => {
    ...
    try{
      const resp = await axios.delete(`${process.env.REACT_APP_BASE_URL}api/v1/coupon/${itemid}`);
      if (resp.status === 200 && resp.data.success) {
